...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        self.out_norm = LayerNorm(dim, use_bias) # TODO: Define the output layer normalization. Use the LayerNorm class defined in modeling/transformer_layers.py
    ...
    def forward_model(self, x: torch.LongTensor) -> torch.Tensor:
        ...
        mask = torch.tril(torch.ones(L, L, device=x.device)).bool().view(1, L, L)
    ...
    def generate(
        ...
        for _ in range(self.max_seq_len - len(context)):
            ...
            current_tokens = torch.cat((current_tokens, next_token[:, None]), dim=1)

            # Break if the end-of-sequence token is generated
            if eos_idx is not None and (next_token == eos_idx).any():
                break
